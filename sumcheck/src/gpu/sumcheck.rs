...
use ff::PrimeField;
use transcript_utils::transcript::FieldTranscriptWrite;
...
impl<F: PrimeField + FromFieldBinding<F> + ToFieldBinding<F>> GPUApiWrapper<F> {
    ...
    pub(crate) fn eval_at_k_and_combine(
        ...
        let round_eval_values = self
            ...
            .map_err(|e| LibraryError::Driver(e))?;
        transcript.write_field_elements(&round_eval_values).map_err(|e| LibraryError::Transcript)?;
        Ok(())
    }
...
//     use crate::{
//         cpu, fieldbinding::ToFieldBinding, GPUApiWrapper, LibraryError, MULTILINEAR_PTX,
//         SUMCHECK_PTX,
//     };
