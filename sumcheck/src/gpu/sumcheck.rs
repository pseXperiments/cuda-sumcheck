        ...
        for k in 0..max_degree + 1 {
            ...
            unsafe {
                fold_into_half.launch(
                    launch_config,
                    (initial_poly_num_vars - round, 1 << initial_poly_num_vars, polys, &mut *buf, &device_k),
                )?;
            };
...
mod tests {
    ...
    fn test_prove_sumcheck() -> Result<(), DriverError> {
        ...
        let mut challenges = gpu_api_wrapper.copy_to_device(&vec![Fr::ZERO; num_vars])?;
        let mut round_evals =
            gpu_api_wrapper.copy_to_device(&vec![Fr::ZERO; num_vars * (max_degree + 1)])?;
        let round_evals_view = RefCell::new(round_evals.slice_mut(..));
